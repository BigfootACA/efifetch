...
{
	if(g.pstart) free(g.pstart);
	if(g.prog) free(g.prog);
	g.error = message;
}
...
static int nextrune(void)
{
	...
	if (g.yychar == '\\') {
		g.source += chartorune(&g.yychar, g.source);
		switch (g.yychar) {
		case 0: die("unterminated escape sequence"); return -1;
...
static int lexcount(void)
{
	...
	if (g.yymin == 0) return -1;
	g.yychar = *g.source++;
	while (g.yychar != ',' && g.yychar != '}') {
		v = dec(g.yychar);
		if (v == 0) return -1;
		g.yymin = g.yymin * 10 + v;
		g.yychar = *g.source++;
	}
	if (g.yymin >= REPINF) {
	...
	if (g.yychar == ',') {
		...
		if (g.yychar == '}') {
			...
			if (g.yymax == 0) return -1;
			g.yychar = *g.source++;
			while (g.yychar != '}') {
				v = dec(g.yychar);
				if (v == 0) return -1;
				g.yymax = g.yymax * 10 + v;
				g.yychar = *g.source++;
			}
			if (g.yymax >= REPINF) {
				die("numeric overflow");
...
static unsigned int count(Renode *node)
{
	...
	case P_REP:
		...
		if (min == max) n = v1 * min;
		else if (max < REPINF) n = v1 * max + (max - min);
		else n = v1 * (min + 1) + 2;
		if (n > MAXPROG) {
...
Reprog *regexp_comp(const char *pattern, int cflags, const char **errorp)
{
	...
	if (!g.prog->start){
		die("cannot allocate regular expression program");
		return NULL;
	}
...
{
	if (prog) {
		if(prog->start) free(prog->start);
		free(prog);
	}
}

int regexp_match(const char *regex, const char *str, int flags) {
	Reprog *prog;
	prog = regexp_comp(regex, flags, NULL);
	if (!prog) return 0;
	int ret = regexp_exec(prog, str, NULL, 0);
	regexp_free(prog);
	return ret;
}
